...
data = {
    'client_number': [generate_client_number() for _ in range(10000)],
    'balance': [generate_balance() for _ in range(10000)],
    'last_activity': [generate_date() for _ in range(10000)],
    'Monthly_income': [generate_monthly_income() for _ in range(10000)],
    'Monthly_expenses': [generate_monthly_expenses() for _ in range(10000)],
    'Monthly_loan_rate': [generate_monthly_loan_rate() for _ in range(10000)]
}
...
df.to_excel('data_test_final.xlsx', index=False)
